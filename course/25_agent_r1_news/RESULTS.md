...
确实会生成这种“有标签、无内容”的动作。课程现要求 `<think>` 至少包含一个非空字符，
并新增逐动作 `thinking_score` 和显式思考覆盖率。旧 SFT/GRPO 训练已经启动，不能把事后
修正冒充训练时就使用了该奖励，因此下方长跑仍按旧协议如实记录；训练结束后的统一动态
评测使用严格协议，未来从仓库重跑则会同时优化非空思考。若严格评测显示覆盖率明显不足，
应把它作为一项可复现的协议退化，而不是用动作正确率掩盖。
