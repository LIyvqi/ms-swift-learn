...
已使用 2880 条全量三任务数据启动 2 轮训练，共 2880 step；参数为 batch 6、G=3、generation batch 12、temperature 0.8、每轮最多 160 token、最多 6 轮，并开启梯度检查点。平台执行会话在一次长跑中发出过外部 `SIGTERM`，因此保存间隔由 240 缩短为 120 step，最多保留 24 个带优化器状态的可恢复检查点；训练后将把同一 120 条动态验证结果补到这里。
