...
第二轮截至 1920 step 的连续窗口如下。所有 checkpoint 权重与优化器张量均为有限值；“尖峰”列统计裁剪前 `loss>1` 或 `grad_norm>1000` 的 step 数，不能单独解释为更新已失控。
...
1734 step 出现一次 `loss=7.63`、`grad_norm=33792` 的裁剪前尖峰，后续 step 立即恢复，1800 和 1920 checkpoint 的参数检查均通过。更值得关注的是窗口层面的共同变化：决策和协议奖励下降、KL 与平均轮数上升。它可能表示在线策略开始偏离稳定区域，也可能受第二轮当前任务批次影响；因此增加 1920 为正式动态评测节点，但在固定选择集结果出来前不把训练内信号写成泛化结论。
