...
第二轮截至 2280 step 的连续窗口如下。所有 checkpoint 权重与优化器张量均为有限值；“尖峰”列统计裁剪前 `loss>1` 或 `grad_norm>1000` 的 step 数，不能单独解释为更新已失控。
...
1734 step 出现一次 `loss=7.63`、`grad_norm=33792` 的裁剪前尖峰，后续 step 立即恢复；2041～2280 没有 `loss>1` 或 `grad_norm>1000`，2161～2280 的局部最大值分别为 0.240 和 692。2280 checkpoint 的 372 个 LoRA 张量与 1116 个优化器张量也全部有限。更值得关注的是窗口层面的共同变化：决策和协议奖励仍低于第二轮早段，KL 维持在约 0.44～0.48。它可能表示在线策略开始偏离稳定区域，也可能受第二轮当前任务批次影响；因此 1920 和 2160 都进入正式动态评测候选，但在固定选择集结果出来前不把训练内信号写成泛化结论。
