...
差异在后 30 个 rollout 更明显：原参数 KL 为 0.532、decision reward 为 0.679，新参数分别为 0.261 和 0.788。检索与组合奖励基本持平，因此优势主要来自策略更新更稳定，而不是任务批次更容易。课程默认值已切换为胜出参数；正式两轮分支从共同祖先 600 step 重新运行，配对试验本身不混入正式曲线。
