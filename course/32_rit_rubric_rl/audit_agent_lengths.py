#!/usr/bin/env python3
"""使用真实 Qwen 聊天模板审计 Agent 多轮轨迹 token 长度。"""
...
def 主程序() -> None:
    parser = argparse.ArgumentParser(description="审计极简审核 Agent 数据长度")
    parser.add_argument(
        "--dataset",
        type=Path,
        default=项目根目录 / "datasets/rit_audit_agent/sft_train.jsonl",
    )
    parser.add_argument(
    ...
    if over_limit:
        raise SystemExit("存在超过 Agent 训练 max_length 的样本")
