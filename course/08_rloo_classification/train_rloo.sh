...
latest_checkpoint() {
  ...
  if [[ -z "${checkpoint}" ]]; then
    echo "找不到分类 SFT 检查点：${directory}" >&2
    return 1
  fi
  printf '%s\n' "${checkpoint}"
}
...
SFT_ROOT="${SFT_OUTPUT:-${ROOT}/outputs/08_rloo_classification/sft}"
SFT_ADAPTER="${SFT_ADAPTER:-$(latest_checkpoint "${SFT_ROOT}")}"
RLOO_STEPS="${RLOO_STEPS:-100}"
OUTPUT="${RLOO_OUTPUT:-${ROOT}/outputs/08_rloo_classification/rloo_${RLOO_STEPS}step}"

if [[ "${SMOKE:-0}" == "1" ]]; then
  TRAIN_DATA="${DATA}/rl_smoke.jsonl"
  RLOO_STEPS=1
  OUTPUT="${RLOO_OUTPUT:-${ROOT}/outputs/08_rloo_classification/rloo_smoke}"
else
...
swift rlhf \
  ...
  --use_vllm true \
  --vllm_mode colocate \
  --vllm_gpu_memory_utilization "${VLLM_MEMORY:-0.35}" \
  ...
  --per_device_train_batch_size "${RL_BATCH:-16}" \
  --gradient_accumulation_steps 1 \
  --learning_rate "${RLOO_LEARNING_RATE:-5e-6}" \
  --beta "${RLOO_BETA:-0.001}" \
  --max_grad_norm 1.0 \
  --max_length 768 \
