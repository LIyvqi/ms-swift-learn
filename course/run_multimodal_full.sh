...
# Direct 输出很短，将 12 条 rollout 集中生成后分成两个 batch 更新。
# 实测 20/24 条图像集中预处理反而降低吞吐，12 条是速度与显存的更好折中。
DIRECT_OPD_GENERATION_BATCH="${MM_DIRECT_OPD_GENERATION_BATCH:-12}"
...
if [[ "${START_STAGE}" -le 6 ]]; then
  记录步骤 "04 Direct 多模态 OPD：${RL_STEPS} step，batch=${DIRECT_OPD_BATCH_SIZE}，生成batch=${DIRECT_OPD_GENERATION_BATCH}"
  STEPS="${RL_STEPS}" STYLE=direct RUN_TAG="full_${RL_STEPS}step" \
  RL_BATCH="${DIRECT_OPD_BATCH_SIZE}" GENERATION_BATCH="${DIRECT_OPD_GENERATION_BATCH}" \
